<template>
	<main class="advert" v-if="advert">
		<img :src="'https://theorige.com' + advert.attributes.covers.data[0].attributes.formats.medium.url" alt="Couverture de l'annonce" />
...
onBeforeMount(async () => {
	const { data } = await axios.get('https://theorige.com/api/colocations/' + route.params.id + '?populate=*', {
		headers: {
